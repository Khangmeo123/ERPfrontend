...
class TranslateCLI {
  ...
  /**
   * Generate translate for libraries
   */
  generateTranslateForLibraries() {
    const directories: string[] = [
      'selects',
    ];
    directories.forEach((directory: string) => {
      const input: string = `./projects/jaja-libraries/src/${directory}`;
      const outputFiles: string[] = this.languages.map((language: string) => {
        return `./src/assets/i18n/${directory}.libraries.${language}.json`;
      });
      const output: string = outputFiles.join(' ');
      this.translate(input, output);
      this.deleteGeneralInSpecificFiles(outputFiles);
  ...
  .action((modulePath: string) => {
    ...
    switch (modulePath) {
      ...
      case 'rest':
        ...
        break;
      case 'libraries':
        translateCLI.generateTranslateForLibraries();
        break;
      case 'all':
        translateCLI.generateTranslateForLibraries();
