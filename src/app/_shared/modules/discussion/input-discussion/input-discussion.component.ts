...
@Component({
  selector: 'app-input-discussion',
  templateUrl: './input-discussion.component.html',
  styleUrls: ['./input-discussion.component.css'],
})
export class InputDiscussionComponent implements OnInit {
  ...
  TurnOff(event?) {
    ...
    if (this.DataList != null && this.ElemToReplace != null && this.AutoReplaceWhenClick == true) {
      ...
      el.innerHTML = Data['display'] + ' (' + Data['name'] + ')';
  ...
  TurnOffAnother() {
    if (this.isTurnOffAnother == false || InputDiscussionComponent.IsBlock) {
      return;
    }
